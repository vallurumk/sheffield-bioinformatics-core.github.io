...
One of the biggest strengths of working in Projects is the convention of **specifying file paths relative to the project root**. If all file paths in the project are specified relative to this root, code involving paths will direct to the correct locations, even if the Project is moved or shared.
...
Add `eval = F` to the chunk options as installation of packages should usually not be part of workflows.
...
head(byWealth)
write_csv(byWealth, path = here::here("data", "by_wealth.csv"))
```
...
filter(gapminder, continent=="Europe") %>% 
  ...
  write_csv(path = here::here("data", "europe_by_lifeExp.csv"))
